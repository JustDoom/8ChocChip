...
Modified for 8ChocChip and into more C++ compliant code
...
void SHA1Update(SHA1_CTX* context, const unsigned char* data, const uint32_t len) {
    uint32_t i;
...
#else
    for (i = 0; i < 8; i++) {
        finalcount[i] = static_cast<unsigned char>((context->count[(i >= 4 ? 0 : 1)] >> ((3 - (i & 3)) * 8)) & 255);
        /* Endian independent */
    }
#endif
    c = 0200;
...
void SHA1(char* hash_out, const char* str, const uint32_t len) {
    SHA1_CTX ctx;
